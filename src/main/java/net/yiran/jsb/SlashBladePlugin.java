...
public class SlashBladePlugin implements IModPlugin {
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        if (Config.FILTER_DROP.get()) {
            drops = drops.filter(entityDropEntry -> {
                if (!BuiltInRegistries.ENTITY_TYPE.containsKey(entityDropEntry.entityType())) return false;
    ...
    public void registerIngredients(IModIngredientRegistration registration) {
        registration.register(SAIngredient.INSTANCE, SlashArtsRegistry.REGISTRY.stream().toList(), SAIngredient.INSTANCE, SAIngredient.INSTANCE, SlashArtsRegistry.REGISTRY.byNameCodec());
        registration.register(SEIngredient.INSTANCE, SpecialEffectsRegistry.REGISTRY.stream().toList(), SEIngredient.INSTANCE, SEIngredient.INSTANCE, SpecialEffectsRegistry.REGISTRY.byNameCodec());
    }
